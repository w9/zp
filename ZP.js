...
ZP.ScaleColorFactor = function (vec_, name_, palette_ = ZP.COLOR_PALETTE) {
  ...
  let _change_level = function (l, new_dimmed_) {
    ...
    _legend.dispatchEvent(
      ZP.legend_action_event({
        type: "opacity",
        indices: _indices[l],
        opacity: new_dimmed_ ? "dim" : 1,
      })
    );
    _legend.dispatchEvent(
      ZP.legend_action_event({
  ...
  _legend.addEventListener("dblclick", function (e) {
    _toggleAllLevels();
  });
  ...
  let _create_legend_item = function (level_, color_) {
    ...
    item_DIV.innerHTML = `<span class="color-patch" style="background-color: ${color_}"></span>${_format_legend_text(
      level_
    )}`;
    item_DIV.addEventListener("click", function (e) {
      e.ctrlKey ? _onlyShowOneLevel(level_) : _toggleLevel(level_);
    });
    item_DIV.addEventListener("dblclick", function (e) {
      e.stopPropagation();
    });
...
ZP.ScaleColorString = function (vec_, name_, palette_) {
  ...
  return new ZP.ScaleColorFactor(
    { data: _vec, levels: _levels },
    name_,
    palette_
  );
};
...
ZP.ScaleColorBoolean = function (vec_, name_) {
  ...
  let _this = new ZP.ScaleColorFactor({ data: _vec, levels: _levels }, name_, [
    "#ff0000",
    "#0077ff",
  ]);
...
ZP.ScaleColorDensity = function () {
  ...
  let _change_opacity_to = function (opacity_) {
    _legend.dispatchEvent(
      ZP.legend_action_event({ type: "opacity", opacity: opacity_ })
    );
  };

  let _opacity_slider = _legend.querySelector("#scn-opacity-slider");
  _opacity_slider.addEventListener("input", (e) =>
    _change_opacity_to(_opacity_slider.value, false)
  );
...
ZP.ScaleColorNumeric = function (vec_, name_, options_) {
  ...
  _legend.innerHTML =
    ...
    `<stop stop-color="rgba(${ZP.hex_to_rgba(
      options_.cc_low,
      _use_transparency ? ZP.CC_OPACITY_LOW : 1
    ).join(",")})" offset="0%"/>` +
    `<stop stop-color="rgba(${ZP.hex_to_rgba(
      options_.cc_high,
      ZP.CC_OPACITY_HIGH
    ...
    `<line x1="0" y1="${ZP.CC_PATCH_HEIGHT - 0.5}" x2="${
      ZP.CC_PATCH_WIDTH + 10
    }" y2="${ZP.CC_PATCH_HEIGHT - 0.5}" stroke="black"/>` +
    `<text x="${
      30 + ZP.CC_PATCH_LABEL_MARGIN
    }" y="0.5" alignment-baseline="middle" text-anchor="start" font-size="0.8em">${_high}</text>` +
    `<text x="${30 + ZP.CC_PATCH_LABEL_MARGIN}" y="${
      ZP.CC_PATCH_HEIGHT - 0.5
    }" alignment-baseline="middle" text-anchor="start" font-size="0.8em">${_low}</text>` +
    `</svg>`;
  ...
  _gradient_strip_RECT.addEventListener("click", function (e) {
    ...
    _gradient.children[0].setAttribute(
      "stop-color",
      `rgba(${ZP.hex_to_rgba(
        options_.cc_low,
        _use_transparency ? ZP.CC_OPACITY_LOW : 1
  ...
  _legend.reset = function () {
    _legend.dispatchEvent(ZP.legend_action_event({ type: "selectability" }));
    _legend.dispatchEvent(
      ZP.legend_action_event({ type: "color", color: _colors })
    );
    if (_use_transparency) {
      _legend.dispatchEvent(
        ZP.legend_action_event({ type: "opacity", opacity: _opacities })
      );
    } else {
...
ZP.Aes = function (data_, mappings_, options_) {
  ...
  _title_DIV.innerText = Object.keys(_current.coord)
    .map((x) => _current.coord[x].name)
    .join(", ");
...
ZP.ZP = function (el_, width_, height_) {
  ...
  let _ortho_camera = new THREE.OrthographicCamera(
    ar * -s,
    ar * s,
    s,
    -s,
  ...
  toggleOrthoButton.title =
    "toggle between orthographic and perspective camera";
  ...
  this.plot = function (data_, mappings_, options_) {
    ...
    _orbit = new THREE.OrbitControls(
      _camera,
      _renderer.domElement,
      new THREE.Vector3(0, 0, 0)
    );
    _orbit.addEventListener("userRotate", function (e) {
      _ortho = "none";
    });
    ...
    _ortho_orbit = new THREE.OrbitControls(
      _ortho_camera,
      _renderer.domElement,
      new THREE.Vector3(0, 0, 0)
    );
    _ortho_orbit.addEventListener("userRotate", function (e) {
      _ortho = "none";
    });
    _ortho_orbit.mouseButtons = {
      ORBIT: null,
    ...
    let _update_arena = function () {
      ...
      for (let i in _points) {
        ...
        new TWEEN.Tween(a)
          .to(b, options_.animation_duration)
          .easing(TWEEN.Easing.Exponential.Out)
          .onUpdate(function () {
            _points[i].position.set(this.y, this.z, this.x);
          })
          .start();
      ...
      for (let i of inds_) {
        ...
        if (animation_) {
          ...
          new TWEEN.Tween(a)
            ...
            .easing(TWEEN.Easing.Exponential.Out)
            .onUpdate(function () {
              _points[i].material.color = new THREE.Color(ZP.rgba_to_hex(this));
            })
            .start();
        } else {
      ...
      for (let i of inds_) {
        ...
        if (animation_) {
          ...
          new TWEEN.Tween(a)
            ...
            .easing(TWEEN.Easing.Exponential.Out)
            .onUpdate(function () {
              _points[i].material.opacity = this.opacity;
            })
            .start();
        } else {
    ...
    if (options_.title) {
      document.title = "ZP - " + options_.title;
    }
    if (options_.debug) {
      console.log("data_ = ", data_);
    }
    if (options_.debug) {
      console.log("mappings_ = ", mappings_);
    }

    _aes = new ZP.Aes(data_, mappings_, options_);
    if (options_.debug) {
      console.log("_aes = ", _aes);
    }

    _data_rows = ZP.colsToRows(data_);
    if (options_.debug) {
      console.log("_data_rows = ", _data_rows);
    }

    _data_indices = ZP.range0(_data_rows.length);
    let l = _data_indices.length;
    if (options_.dot_size == "auto")
      options_.dot_size = Math.cbrt(
        Math.pow(2, 3) + Math.pow(10, 3) / (l / 20 + 1)
      );
    if (options_.dimmed_opacity == "auto")
      options_.dimmed_opacity = Math.cbrt(
        Math.pow(0.01, 3) + Math.pow(0.1, 3) / (l / 5000 + 1)
      );
    ...
    for (let i of _data_indices) {
      ...
      let discMtrl = new THREE.SpriteMaterial({
        map: _disc_txtr,
    ...
    _crosshairs.tweenObj = {
      size: options_.dot_size * ZP.CROSSHAIR_SIZE_FACTOR,
    };
    _crosshairs.tween = new TWEEN.Tween(_crosshairs.tweenObj);
    _crosshairs.tween
      .to({ size: options_.dot_size * ZP.CROSSHAIR_SIZE_FACTOR * 1.4 }, 800)
      .easing(TWEEN.Easing.Sinusoidal.InOut)
      .repeat(Infinity)
      .yoyo(true)
      .onUpdate(function () {
        _crosshairs.scale.set(this.size, this.size, 1);
      })
      .start();
      ...
      switch (d.type) {
        case "color":
          ...
          if (typeof d.animation === "undefined")
            d.animation = options_.animation;
    ...
    window.addEventListener("keydown", function (e) {
      switch (e.key) {
        case ZP.KEY_COORD_PREV:
          _prev_coord_BUTTON.dispatchEvent(new Event("click"));
          break;
        case ZP.KEY_COORD_NEXT:
          _next_coord_BUTTON.dispatchEvent(new Event("click"));
          break;
        case ZP.KEY_COLOR_PREV:
          _prev_color_BUTTON.dispatchEvent(new Event("click"));
          break;
        case ZP.KEY_COLOR_NEXT:
          _next_color_BUTTON.dispatchEvent(new Event("click"));
          break;
        case ZP.KEY_VIEW_RESET:
          resetCameraButton.dispatchEvent(new Event("click"));
          break;
        case ZP.KEY_TOGGLE_ORTHO_VIEWS:
          toggleOrthoButton.dispatchEvent(new Event("click"));
          break;
      }
    });
