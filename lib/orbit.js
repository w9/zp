...
THREE.OrbitControls = function ( object, domElement, target ) {
	...
	// The four arrow keys
	this.panKeys    = { LEFT: 'ArrowLeft' , UP: 'ArrowUp' , RIGHT: 'ArrowRight' , BOTTOM: 'ArrowDown' };
	this.rotateKeys = { LEFT: 'a'         , UP: 'w'       , RIGHT: 'd'          , BOTTOM: 's' };
  ...
  this.moveTo = function (position, target) {
	...
	function handleKeyDown( event ) {
		...
		switch ( event.key ) {
			...
			case scope.rotateKeys.UP:
        rotateUp(   Math.PI * scope.keyRotateSpeed );
				scope.update();
				break;

			case scope.rotateKeys.BOTTOM:
        rotateUp( - Math.PI * scope.keyRotateSpeed );
				scope.update();
				break;

			case scope.rotateKeys.LEFT:
        rotateLeft(   Math.PI * scope.keyRotateSpeed );
				scope.update();
				break;

			case scope.rotateKeys.RIGHT:
        rotateLeft( - Math.PI * scope.keyRotateSpeed );
				scope.update();
				break;

			case scope.panKeys.UP:
				pan( 0,   scope.keyPanSpeed );
				scope.update();
				break;

			case scope.panKeys.BOTTOM:
				pan( 0, - scope.keyPanSpeed );
				scope.update();
				break;

			case scope.panKeys.LEFT:
				pan(   scope.keyPanSpeed, 0 );
				scope.update();
				break;

			case scope.panKeys.RIGHT:
				pan( - scope.keyPanSpeed, 0 );
				scope.update();
				break;
